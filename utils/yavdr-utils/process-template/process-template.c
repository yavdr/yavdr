...
{
  printf("USAGE\n\t%s [--owner <uid>] [--group <gid>] [--mode <permisson mask>] [--output <output file>]\n\t\t[--data <key=value>] [--database <database file>] <filename>\n", name);
  printf("\n\t%s [-h|--help]\n", name);
  printf("\nDESCRIPTION\n\t%s generates files from template snippets found under the directory\n\t%s with optional templates under %s\n\tusing the template language clearsilver and a hdf database at location %s.\n", name, TEMPLATEPATH, TEMPLATECUSTOMPATH, YAVDRDB);
  printf("\nOPTIONS\n\t--owner <uid>\t\t\tGenerated file gets the owner <uid>\n");
  printf("\t--group <gid>\t\t\tGenerated file gets the group <gid>\n");
  printf("\t--mode <permission mask>\tGenerated file gets the permission <permisson mask>\n");
  printf("\t--output <output file>\t\tGenerated file uses the corresponding templates for <filename>\n\t\t\t\t\tbut is written to <output file>. Useful for files in different\n\t\t\t\t\thome directories generated by the same template.\n");
  printf("\t--data <key=value>\t\tAdditional database key value pairs for usage in the templates.\n");;
  printf("\t--database <database file>\tNormally the database %s is used. This can be\n\t\t\t\t\toverwritten by this option.\n", YAVDRDB); 
}
